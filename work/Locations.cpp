...
void chooseLocation() {
    ...
    // Spawn a mob in the selected location
    Mob mob = spawnRandomMob(location);
    if (mob.name != "None") {
        cout << "\nA wild " << mob.name << " appears!" << endl;
        cout << mob.name << " HP: " << mob.hp << ", Attack: " << mob.attackPower << ", Defense: " << mob.defense << endl;
    }
}
